...
!function(e, t, n) {
    ...
    function p(e, n, r, o) {
        var i, a, s, c, u = "modernizr", f = l("div"), p = d();
        if (parseInt(r, 10))
            for (; r--; )
                s = l("div"),
                s.id = o ? o[r] : u + (r + 1),
    ...
    function v(e, t, o, i) {
        ...
        for (var f, d, p, m, v, g = ["modernizr", "tspan"]; !k.style; )
            f = !0,
            k.modElem = l(g.shift()),
    ...
    function g(e, t, n, o, i) {
        ...
        return r(t, "string") || r(t, "undefined") ? v(s, t, o, i) : (s = (e + " " + _.join(a + " ") + a).split(" "),
        f(s, t, n))
    }
    function y(e, t, r) {
        return g(e, n, n, t, r)
    ...
    x || !function(e, t) {
        ...
        function c(e) {
            e || (e = t);
            var r = i(e);
            return !w.shivCSS || u || r.hasCSS || (r.hasCSS = !!n(e, "article,aside,dialog,figcaption,figure,footer,header,hgroup,main,nav,section{display:block}mark{background:#FF0;color:#000}template{display:none}")),
            f || l(e, r),
            e
        }
    ...
    var A = C._config.usePrefixes ? T.split(" ") : [];
    C._cssomPrefixes = A;
    var z = function(t) {
    ...
    C.prefixed = function(e, t, n) {
        return 0 === e.indexOf("@") ? z(e) : (-1 != e.indexOf("-") && (e = s(e)),
        t ? g(e, t, n) : g(e, "pfx"))
    }
    ;
    C.testAllProps = y,
...
(function(NS) {
    ...
    NS.onDebounced = function(events, handler, trigger) {
        var splits = events.split('.');
        var event_type = splits[0];
        var event_namespace = splits[1];
        if (!event_namespace) {
        ...
        if (!(event_namespace in handlers[event_type])) {
            handlers[event_type][event_namespace] = []
        }
        handlers[event_type][event_namespace].push(handler);
        if (trigger) {
            queue_resize();
            queue_scroll();
        }
    ...
    ;
    NS.offDebounced = function(events, handler) {
        var splits = events.split('.');
        var event_type = splits[0];
        var event_namespace = splits[1];
        if (!event_namespace) {
        ...
        if (handler) {
            var ns_handlers = handlers[event_type][event_namespace]
            for (var i = 0; i < ns_handlers.length; ) {
                if (ns_handlers[i] === handler) {
                    var rest = ns_handlers.slice(i + 1);
                    ns_handlers.length = i;
                    ns_handlers.push.apply(ns_handlers, rest);
                } else {
                    i++;
                }
            }
        } else {
            delete handlers[event_type][event_namespace];
        }
    }
    ;
    function handle_resize() {
        ...
        var namespaces = handlers['resize'];
        for (var namespace in namespaces) {
            for (var i = 0, l = namespaces[namespace].length; i < l; i++) {
                namespaces[namespace][i](win_width, win_height);
            }
        }
    ...
    ;function handle_scroll() {
        ...
        var namespaces = handlers['scroll'];
        for (var namespace in namespaces) {
            for (var i = 0, l = namespaces[namespace].length; i < l; i++) {
                namespaces[namespace][i](scroll_top, scroll_left);
            }
        }
...
(function(NS) {
    'use strict';
    NS.transitionend = (function(transition) {
    ...
    NS.get_match = function(str, re, i) {
    ...
    ;
    $.is_tablet = function() {
        return $(window).width() <= NS.tablet_max && Modernizr.touchevents;
    }
    $.is_phone = function() {
        return $(window).width() <= NS.phone_max && Modernizr.touchevents;
    }
    $.fn.has_one_of_classes = function() {
        for (var i = 0; i < arguments.length; i++) {
            var classes = arguments[i].split(' ');
            for (var j = 0; j < classes.length; j++) {
                if (this.hasClass($.trim(classes[j]))) {
                    return true;
                }
            }
        }
    ...
    ;
    $.fn.has_classes = function() {
        var classes = '';
        for (var i = 0; i < arguments.length; i++) {
            var split = arguments[i].split(' ');
            for (var j = 0; j < split.length; j++) {
                if ($.trim(split[j])) {
                    classes += '.' + $.trim(split[j]);
                }
            }
        }
        if (classes) {
            return this.length == this.filter(classes).length;
        } else {
            return true;
        }
    }
    ;
    $.fn.images_loaded = function(timeout) {
...
(function(NS) {
    ...
    $.prevent_scroll = function(hide_scrollbar) {
        var assign = hide_scrollbar ? 'hide_scroll' : 'disable_scroll';
        var remove = hide_scrollbar ? 'disable_scroll' : 'hide_scroll';
        if (!$('body').hasClass(assign)) {
            var top = Math.max($('body').scrollTop(), $('html').scrollTop());
            $('body').addClass(assign).removeClass(remove);
            $('#outer').scrollTop(top);
        }
    }
    ;
    $.allow_scroll = function() {
        if ($('body').has_one_of_classes('disable_scroll', 'hide_scroll')) {
            var top = $('#outer').scrollTop();
            $('body').removeClass('disable_scroll').removeClass('hide_scroll');
    ...
    ;
    $.polite_scroll_to = function(val, duration, callback) {
        var scroll_el = $('html, body');
...
(function(NS) {
    ...
    ;
    function class_based_transition($el, from_classes, to_classes, callback) {
        var add = to_classes || ''
        var remove = from_classes || ''
        if (Modernizr.csstransitions) {
            var foo = $el.has_one_of_classes(remove);
            var bar = !$el.has_classes(add);
            if (foo || bar) {
                $el.one(NS.transitionend, function() {
    ...
    $.fn.transition_to = function(classes, callback) {
        $(this).each(function() {
            class_based_transition($(this), undefined, classes, callback)
        });
        return this;
    }
    ;
    $.fn.transition_from = function(classes, callback) {
        $(this).each(function() {
            class_based_transition($(this), classes, undefined, callback)
        });
        return this;
    }
    ;
    $.fn.from_transition_to = function(from_classes, to_classes, callback) {
        $(this).each(function() {
            class_based_transition($(this), from_classes, to_classes, callback)
        });
        return this;
    }
    ;
    $.fn.css_fade_in = function(callback) {
        var me = $(this);
        me.from_transition_to('transparent', 'opaque', callback);
        return this;
    }
    ;
    $.fn.css_fade_out = function(callback) {
        var me = $(this);
        me.from_transition_to('opaque', 'transparent', callback);
        return this;
    }
    ;
})(window.utilities);
